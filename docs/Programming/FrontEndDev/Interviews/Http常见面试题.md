...
![Image 3: 三个部分](./imgs/1747213235583.png)
...
## HTTP 常见的状态码有哪些
![HTTP 状态码 ](./imgs/1747213236349.png)
...
![Image 7](./imgs/1747213235911.png)

__Content-Length__ : 服务器在返回数据时，会有 `Content-Length` 字段，表明本次回应的数据长度。  
`Content-Length: 1000`    

![Image 8: a diagram showing the contents of a web page in chinese](./imgs/1747213235787.png)


__Connection__ : 字段最常用于客户端要求服务器使用「HTTP 长连接」机制，以便其他请求复用。   
`Connection: keep-alive`   

![Image 9: a diagram showing how to connect to the internet](./imgs/1747213236198.png)

HTTP 长连接的特点是，只要任意一端没有明确提出断开连接，则保持 TCP 连接状态。   
HTTP/1.1 版本的默认连接都是长连接，但为了兼容老版本的 HTTP，需要指定 `Connection: Keep-Alive` 才能开启长连接。  

![Image 10: HTTP 长连接](./imgs/1747213235581.png)
...
![Image 11: a diagram of a content type that is used in chinese](./imgs/1747213235826.png)

__Content-Encoding__ : 字段说明数据的压缩方法。表示服务器返回的数据使用了什么压缩格式.   
`Content-Encoding: gzip`    
客户端发起请求时，可能通过 `Accept-Encoding: gzip, deflate` 来声明自己可以接受哪些压缩方法。

![Image 12: a diagram showing the process of encrypting and decrypting a file](./imgs/1747213235804.png)
...
![Image 13: GET 请求](./imgs/1747213235837.png)
...
![Image 14: POST 请求](./imgs/1747213235665.png)
...
![Image 15: A web inspector showing the request URL, method, status code and the remote address](./imgs/1747213235995.png)
...
![Image 16: a diagram showing how to connect to a server in chinese](./imgs/1747213236319.png)   
...
![Image 18: Cookie 技术](./imgs/1747213236123.png)
...
![Image 20: 短连接与长连接](./imgs/1747213235978.png)
...
![Image 21: 管道网络传输](./imgs/1747213235690.png)
...
![Image 22: 队头阻塞](./imgs/1747213236168.png)
...
![Image 23: HTTP 与 HTTPS 网络层](./imgs/1747213235399.jpeg)
...
![Image 24: 混合加密](./imgs/1747213236116.png)
...
![Image 25: a diagram showing the process of creating a file in a korean language](./imgs/1747213236238.png)
...
![Image 26: The image illustrates the digital signature process in Chinese](./imgs/1747213236263.png)
...
![Image 27: 数子证书工作流程](./imgs/1747213235990.jpeg)
...
![Image 28: HTTPS 连接建立过程](./imgs/1747213235881.jpeg)
...
![Image 29: a diagram of a system with chinese characters](./imgs/1747213236017.png)
...
![Image 30: a screenshot of a web page with chinese and chinese text](./imgs/1747213235652.png)
...
![Image 31: a diagram showing the process of a globalsign root](./imgs/1747213235779.png)
...
![Image 34: a diagram showing the structure of a chinese company](./imgs/1747213236421.png)
...
![Image 36: a diagram showing the process of creating a chinese website](./imgs/1747213236074.png)
...
![Image 38: a diagram of a chinese language with chinese characters](./imgs/1747213236004.png)
...
![Image 39: HTT/1 ~ HTTP/2](./imgs/1747213235549.jpeg)
...
![Image 40: HTTP/1 与 HTTP/2 ](./imgs/1747213236176.png)
...
![Image 44: a diagram of a message frame and a response message frame](./imgs/1747213235937.png)
...
![Image 45: a diagram of an http2 connection](./imgs/1747213235607.jpeg)
...
![Image 46: a diagram of the http2 connection](./imgs/1747213235987.png)
...
![Image 47: Diagram showing the difference between browser loading with and without push](./imgs/1747213235602.png)
...
![Image 48: tcp tcp tcp tcp tcp tcp tcp t](./imgs/1747213235572.jpeg)

### HTTP/3 做了哪些优化

解决了**`队头阻塞`**问题，由于`HTTP/1.1` 和 `HTTP/2 ` 底层传输协议为 `TCP/IP` 协议，它始终存在**队头阻塞**问题。　`HTTP/3 `把 HTTP 下层的 `TCP` 协议改成了 `UDP`协议。

![Image 50: HTTP/1 ~ HTTP/3](./imgs/1747213235769.jpeg)
...
![Image 51: a diagram of a cell phone with chinese text on it](./imgs/1747213235654.jpeg)
...
![Image 52: TCP HTTPS（TLS/1.3） 和 QUIC HTTPS ](./imgs/1747213235782.jpeg)

在第二次连接的时候，应用数据包可以和 QUIC 握手信息（连接信息 + TLS 信息）一起发送，达到 0-RTT 的效果。如下图右边部分，HTTP/3 当会话恢复时，有效负载数据与第一个数据包一起发送，可以做到 0-RTT（下图的右下角）：

![Image 53: a diagram showing different types of network connections](./imgs/1747213235676.png)

__3. 连接迁移__

基于 TCP 传输协议的 HTTP 协议，由于是通过四元组（源 IP、源端口、目的 IP、目的端口）确定一条 TCP 连接。

![Image 54: TCP 四元组](./imgs/1747213235547.png)
